      ...
      <div class="form-and-image">
        <div class="form-content">
          <el-form :model="data.form" :rules="formRules" ref="formRef">
            ...
            <el-form-item>
              <el-button type="primary" class="custom-button large-button" @click="register">注 册</el-button>
            </el-form-item>

          </el-form>

          <div class="login-link">
            已有账号？<router-link to="/login" class="white-link">点击登录</router-link>
          </div>
  ...
  formRef.value.validate(valid => {
    if (valid) {
      request.post('/register', data.form).then(res => {
        if (res.code === '200') {
          ElMessage.success('注册成功，请登录后补全个人信息')
          // 短暂延迟后跳转到登录页面
          setTimeout(() => {
            router.push('/login')
          }, 1000)
